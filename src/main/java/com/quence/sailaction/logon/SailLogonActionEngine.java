...
public class SailLogonActionEngine implements ActionEngine {
    ...
    private static boolean checkParms(String clientName) throws FileNotFoundException, IOException {
        ...
        if (!f.exists() || f.isDirectory()) {
            System.out.println(dtf.format(now()) + " SailLogonActionEngine:checkParms: ini file not found");
        }
        String tc = Ini.getValue(clientName, "TC");
        if (tc == null) {
            System.out.println(dtf.format(now()) + " SailLogonActionEngine:checkParms: TC param not found");
        }
        return true;
    }
}
